...
print("VERSÃO 10.7 ATIVA: Instagram com arte JPEG via Watermark API + título | Facebook + Telegram preservados")
...
def instagram_art_url(post):
    """Gera uma arte pública usando QuickChart Watermark API.

    Estratégia robusta:
    1) cria uma faixa branca JPEG separada, contendo apenas o título;
    2) usa a Watermark API para sobrepor essa faixa à imagem original.

    Isso evita o antigo backgroundImageUrl + datalabels + formatter, que estava
    provocando HTTP 400 no QuickChart.
    """
    ...
    # Faixa branca independente: não depende de backgroundImageUrl nem de
    # formatter JavaScript. O próprio título do Chart.js aceita múltiplas linhas.
    overlay_config = {
        "type": "bar",
        "data": {
            "labels": [""],
    ...
    # A Watermark API é própria para combinar duas imagens públicas e posicionar
    # a segunda na parte inferior. O markRatio=1 faz a faixa ocupar a largura.
    return (
