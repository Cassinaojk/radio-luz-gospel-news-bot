...
print("RÁDIO LUZ GOSPEL - ROBÔ DE NOTÍCIAS 8.2")
    ...
    try:
        while True:
            ...
            for post in data.get("items", []):
                ...
                for match in re.findall(
                    r'RADIO_LUZ_GOSPEL_SOURCE_URL:\s*(https?://[^\s-]+)',
                    content,
                    flags=re.I,
                ):
                    source_urls.add(normalize_url(match))
...
def main():
    print(
        "News Gospel + UAU Gospel + Folha Gospel Música + Guiame Música | "
        "Versão 8.2"
    )
...
def get_article(url):
    ...
    # O News Gospel atual usa blocos de conteúdo que nem sempre são <p>.
    # Primeiro tentamos os parágrafos; se forem insuficientes, usamos o texto
    # real do container da matéria, preservando as quebras de bloco.
    ...
    if len(text) < selfbot.MIN_SOURCE_CHARS or len(ps) < selfbot.MIN_SOURCE_PARAGRAPHS:
        ...
        for line in box.get_text("\n", strip=True).splitlines():
            ...
            if not t:
                continue
            if t.lower() in {"comente","comentários","comentarios","deixe o seu comentário!","deixe o seu comentario!"}:
                continue
            raw_lines.append(t)

        # remove repetições consecutivas geradas por elementos aninhados
        clean_lines=[]
        for t in raw_lines:
            if not clean_lines or t != clean_lines[-1]:
                clean_lines.append(t)
        fallback_text="\n\n".join(clean_lines)
        if len(fallback_text) > len(text):
            text=fallback_text
            ps=[z for z in clean_lines if len(z)>=25]
            print("Fallback de conteúdo ativado:",len(text),"caracteres")

    if len(text)<selfbot.MIN_SOURCE_CHARS or len(ps)<selfbot.MIN_SOURCE_PARAGRAPHS:
        print("Conteúdo insuficiente:",len(text),"caracteres / blocos:",len(ps))
        return None
...
def links(source):
    ...
    if section_only:
        try:
            ...
            print(f"Abrindo seção: {source['url']}\nHTTP: {r.status_code}")
            if r.status_code==200:
                x=BeautifulSoup(r.text,"html.parser")
    ...
    for model in models:
        try:
            ...
            if "429" in msg or "RESOURCE_EXHAUSTED" in msg:
                return None
            if not selfbot.is_transient_gemini_error(e):
                return None
    return None
...
def html(a,d):
    ...
    # A URL da fonte não é exibida ao leitor, mas fica em um marcador
    # HTML invisível para que o robô consiga identificar a matéria original
    # em execuções futuras e evitar republicações.
    source_marker = (
        '<!-- RADIO_LUZ_GOSPEL_SOURCE_URL: '
        + safe_source_url
        + ' -->'
    )
    out += source_marker
...
print("VERSÃO 8.2 ATIVA: + Folha Gospel Música + Guiame Música | Fonte visível sem URL | Originalidade e deduplicação ativas")
selfbot.main()
