...
# Limites corretos: até 3 matérias por execução.
# O workflow roda 3 vezes por dia, totalizando até 9 matérias/dia.
MAX_POSTS_PER_DAY = 3
...
s = requests.Session()
s.headers.update({
    "User-Agent": "Mozilla/5.0 (compatible; RadioLuzGospelBot/4.1)"
})
...
def image_original(x):
    ...
    for u in vals:
        ...
        if (
            ...
            and not any(z in u.lower() for z in (
                "logo", "avatar", "icon", "favicon"
            ))
        ):
            return u
...
def get_article(url):
    ...
    if title.lower() in {
        "lançamentos", "notícias", "noticias", "home",
        "início", "inicio", "últimas notícias", "ultimas noticias"
    }:
...
def links(source):
    ...
    for feed in source["feeds"]:
        ...
        if x:
            for item in x.find_all(["item", "entry"]):
                n = item.find("link")
                u = (
                    n.get("href") or n.get_text(strip=True)
                ) if n else ""
    ...
    if x:
        for a in x.find_all("a", href=True):
            ...
            if not (host == src_host or host.endswith("." + src_host)):
                continue
    ...
    for attempt in range(1, GEMINI_MAX_RETRIES + 1):
        try:
            ...
            if "429" in msg or "RESOURCE_EXHAUSTED" in msg:
                print("Quota Gemini atingida. Sem retry deste modelo.")
                raise

            if (
                not _is_transient_gemini_error(e)
...
"""
    ...
    for model_index, model in enumerate(models_to_try):
        try:
            ...
            if (
                model_index + 1 < len(models_to_try)
                and _is_transient_gemini_error(e)
            ):
                print(
                    f"Modelo {model} indisponível. "
                    f"Tentando {models_to_try[model_index + 1]}..."
                )
                continue
...
def html(a, g):
    ...
    h.append(
        ...
        f'rel="noopener">{a["title"]}</a></small></p>'
    )

    for u in a["videos"]:
        h.append(
            f'<p><iframe src="{u}" width="100%" height="315" '
            f'frameborder="0" allowfullscreen loading="lazy"></iframe></p>'
        )
...
def main():
    print(
        "RÁDIO LUZ GOSPEL | até 3 notícias por execução | "
        "até 3 execuções/dia"
    )
    ...
    # CORREÇÃO PRINCIPAL:
    # antes estava "published < 1", bloqueando tudo após a primeira notícia.
    # Agora permite até 3, respeitando também o limite do Gemini.
    while (
        candidates
        and published < MAX_POSTS_PER_DAY
        and gemini_calls < MAX_GEMINI_TEXT_CALLS_PER_RUN
    ):
        a = candidates.pop(0)

        print("\nVerificando duplicidade:", a["url"])

        if a["url"].rstrip("/") in old:
            continue
        ...
        if not g:
            ...
            if gemini_calls >= MAX_GEMINI_TEXT_CALLS_PER_RUN:
                print(
                    "Limite de chamadas do Gemini atingido "
                    "nesta execução."
                )
                break
