...
print("RÁDIO LUZ GOSPEL - ROBÔ DE NOTÍCIAS 4.0")
...
s=requests.Session()
s.headers.update({"User-Agent":"Mozilla/5.0 (compatible; RadioLuzGospelBot/4.0)"})
gemini_calls=0

def bad_url(u):
    u=(u or "").split("#")[0].strip().lower()
    return not u.startswith(("http://","https://")) or any(x in u for x in BAD)
...
def links(source):
    ...
    if x:
        for a in x.find_all("a",href=True):
            ...
            if "uaugospel.com.br" in source["url"] and "uaugospel.com.br" not in u.lower():continue
            if "newsgospel.com.br" in source["url"] and "newsgospel.com.br" not in u.lower():continue
...
def gemini(a):
    global gemini_calls
    if gemini_calls>=MAX_GEMINI_TEXT_CALLS_PER_RUN:return None
    prompt=f"""
Você é o editor do Rádio Luz Gospel.
Escreva uma matéria jornalística ORIGINAL em português do Brasil usando somente os fatos do texto-fonte.

É OBRIGATÓRIO:
- publicar=true se houver informação suficiente;
- criar um título novo;
- criar resumo;
- criar matéria com 900 a 1800 palavras;
- não inventar informações;
- não dizer que foi escrita por IA;
- não copiar o texto-fonte;
...
"""
    try:
        r=genai.Client(api_key=GEMINI_API_KEY).models.generate_content(
            model=GEMINI_MODEL_TEXT,contents=prompt,
        ...
        if d.get("publicar") is False:
            ...
            return None
        if len(materia)<900:
            print("Resposta do Gemini curta demais.")
            return None
        return d
    except Exception as e:
        gemini_calls+=1
