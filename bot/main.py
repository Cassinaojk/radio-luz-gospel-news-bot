...
def get_article(url):
    global DATE_REJECTED, DATE_MISSING
    x = soup(url)
...
def main():
    ...
    print(f"Descartadas por data antiga/fora da semana: {DATE_REJECTED}")
    print(f"Descartadas por data não identificada: {DATE_MISSING}")
    print(f"Descartadas por não serem música: {music_rejected}")
    print(f"Matérias válidas da semana atual e sobre música: {len(candidates)}")
...
print("VERSÃO 9.2 ATIVA: somente matérias da semana atual e ano atual | música | Folha/Guiame por seção")
selfbot.main()
