...
def _build_ai_providers():
    """Ordem única e definitiva: Gemini principal → Gemini-2 → Groq → Mistral."""
    providers=[]
...
def gemini(a,client):
    ...
    availability = {
        ...
        "Gemini-2": bool(GEMINI_API_KEY_2),
        "Groq": bool(GROQ_API_KEY),
        "Mistral": bool(MISTRAL_API_KEY),
    }
    print("IA configurada: " + " | ".join(f"{k}={'SIM' if v else 'NÃO'}" for k,v in availability.items()))
    if not providers:
...
def source_name(url):
    ...
    if "gospelmais.com" in host:return "Gospel Mais"
    if "exibirgospel.com.br" in host:return "Exibir Gospel"
    if "igospel.org.br" in host:return "iGospel"
...
def source_is_music_page(url):
    ...
    return any(
        key in host
        for key in ("newsgospel.com.br", "uaugospel.com.br", "folhagospel.com", "guiame.com.br", "gospelmais.com", "exibirgospel.com.br", "igospel.org.br")
    )
...
print("VERSÃO 12.22 ATIVA: arquivo consolidado único | Gemini → Gemini-2 → Groq → Mistral | /musicas por conteúdo | Instagram/Facebook/Telegram preservados")
selfbot.main()
