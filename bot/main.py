...
print("RÁDIO LUZ GOSPEL - ROBÔ DE NOTÍCIAS 9.1")
...
s = requests.Session()
s.headers.update({
    "User-Agent": "Mozilla/5.0 (compatible; RadioLuzGospelBot/9.1)"
})
    ...
    try:
        while True:
            ...
            for post in data.get("items", []):
                ...
                # Rádio Luz Gospel 9.1: a URL da fonte fica somente em comentário HTML invisível.
                for match in re.findall(
...
def main():
    print("News Gospel + UAU Gospel + Folha Gospel Música + Guiame Música | Versão 9.1")
...
# ============================================================
# VERSÃO 9.1 - coleta, redação original, deduplicação e log enxuto
# ============================================================
MAX_AGE_DAYS = 30
VERSION = "9.1"
...
s = requests.Session()
s.headers.update({"User-Agent": "Mozilla/5.0 (compatible; RadioLuzGospelBot/9.1)"})
...
print("VERSÃO 9.1 ATIVA: log enxuto | Folha Gospel Música + Guiame Música | fonte sem URL | originalidade e deduplicação")
selfbot.main()
