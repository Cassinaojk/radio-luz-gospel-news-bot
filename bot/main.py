...
def _gemini_request(client, model, prompt):
    ...
    for attempt in range(1, GEMINI_MAX_RETRIES + 1):
        try:
            ...
            msg = str(e).upper()
            quota_error = "429" in msg or "RESOURCE_EXHAUSTED" in msg

            # 429/RESOURCE_EXHAUSTED geralmente indica limite de cota.
            # Esperar poucos segundos não resolve a cota diária; devolvemos
            # o erro para que gemini() tente o modelo alternativo.
            if quota_error:
...
def main():
    print("RÁDIO LUZ GOSPEL | 1 notícia por execução | até 3 execuções/dia")
    ...
    published=0
    while candidates and published<1:
        a=candidates.pop(0)
