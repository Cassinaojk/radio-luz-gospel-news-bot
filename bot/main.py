import os, re, json, requests, time, random, io, contextlib
...
print("RÁDIO LUZ GOSPEL - ROBÔ DE NOTÍCIAS 9.0")
...
def main():
    print("News Gospel + UAU Gospel + Folha Gospel Música + Guiame Música | Versão 9.0")
...
def originality_check(source_text,generated_text):
    ...
    # A similaridade global isolada é fraca para notícias sobre o mesmo fato:
    # nomes, títulos, datas e termos do assunto podem elevar a pontuação sem cópia.
    # Bloqueamos sinais fortes de reprodução literal e usamos a similaridade global
    # apenas quando acompanhada de uma sequência relevante em comum.
    ...
    if ratio>SIMILARITY_MAX and longest>=7:
        return False,f"similaridade textual alta ({ratio:.3f})"
    return True,"OK"
...
print("VERSÃO 9.0 ATIVA: log enxuto | Folha Gospel Música + Guiame Música | fonte sem URL | originalidade e deduplicação")
selfbot.main()
