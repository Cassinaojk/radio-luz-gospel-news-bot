...
s = requests.Session()
s.headers.update({"User-Agent": "Mozilla/5.0 (compatible; RadioLuzGospelBot/7.2)"})
...
def links(source):
    ...
    # remove feeds repetidos preservando a ordem
...
"""
    ...
    for model in models:
        try:
            r=legacy.gemini_request(client,model,prompt); legacy.gemini_calls+=1
            ...
            if not legacy.is_transient_gemini_error(e): return None
    return None
...
print("CORREÇÃO 7.2 ATIVA: compatibilidade Gemini + HTML sem recursão + feeds atuais + conteúdo robusto")
legacy.main()
